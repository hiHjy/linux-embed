...
int irq_gpio_key_fasync (int fd, struct file *file, int on)
{
	struct irq_gpio_key *dev = file->private_data;


	/*

		fasync_helper() 是内核提供的辅助函数，它帮你管理一个 fasync_struct 链表：

		当 on = 1 时：将当前进程添加到 dev->fasync 链表中
...
static int irq_gpio_key_release(struct inode *inode, struct file *filp) {
	...
	irq_gpio_key_fasync(-1, filp, 0);//关闭异步通知
	return 0;
}
...
void timer_func(unsigned long data)
{
	...
	if (value == 0) {
		...
		/*
			0，&dev->fasync给这个链表中的进程
			1，给应用程序发送的信号SIGIO,表示可以进行io操作了
			2，IO类型为POLL_IN 可读
		*/
		printk("释放按键发送信号\n");
